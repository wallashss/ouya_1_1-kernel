...
// static int __nintendo_ble_output_report(struct hid_device *hdev, u8 *data, size_t len, int report_type)
// {
// 	if (report_type == HID_OUTPUT_REPORT)
// 		return hdev->hid_output_raw_report(hdev, data, len, HID_NINTENDO_OUTPUT_REPORT);
	
// 	return hdev->hid_output_raw_report(hdev, data, len, report_type);
// }
...
static int __joycon_hid_send(struct hid_device *hdev, u8 *data, size_t len)
{
	...
	int report_type = HID_OUTPUT_REPORT;

	if (hdev->bus == BUS_BLUETOOTH)
		report_type = HID_NINTENDO_OUTPUT_REPORT;

	ret = hdev->hid_output_raw_report(hdev, buf, len, report_type);
	kfree(buf);
	if (ret < 0)
...
static int joycon_disable_imu(struct joycon_ctlr *ctlr)
{
	struct joycon_subcmd_request *req;
	u8 buffer[sizeof(*req) + 1] = { 0 };
	...
	req = (struct joycon_subcmd_request *)buffer;
	req->subcmd_id = JC_SUBCMD_ENABLE_IMU;
	req->data[0] = 0x00; /* note: 0x00 would disable */

	hid_dbg(ctlr->hdev, "disable IMU\n");
	return joycon_send_subcmd(ctlr, req, 1, HZ);
}
				...
				struct joycon_input_report *rep)
{
	...
	// NOTE(Wallas): This makes USB hangs
	#if ENABLE_NINTENDO_IMU
...
static int joycon_input_create(struct joycon_ctlr *ctlr)
{
	...
	ctlr->imu_input = input_allocate_device();
	if (!ctlr->imu_input)
		return -ENOMEM;
...
static int joycon_read_info(struct joycon_ctlr *ctlr)
{
	...
	ctlr->mac_addr_str = kasprintf(GFP_KERNEL,
			    ...
			    const struct hid_device_id *id)
{
	...
	if ((jc_type_is_procon(ctlr) || jc_type_is_chrggrip(ctlr)) &&
		...
		hid_err(hdev, "detected USB controller\n");
		...
		ret = joycon_disable_imu(ctlr);
		if (ret) {
			hid_err(hdev, "Failed to disable the IMU; ret=%d\n", ret);
			goto err_mutex;
		}
		/*
...
static void nintendo_hid_remove(struct hid_device *hdev)
{
	...
	int i;
	for (i = 0; i < JC_NUM_LEDS; i++) {
		led_classdev_unregister(&ctlr->leds[i]);
		kfree(ctlr->leds[i].name);
	}

	kfree(ctlr->mac_addr_str);
	input_unregister_device(ctlr->input);

	#if ENABLE_NINTENDO_IMU
	devres_free(ctlr->imu_input);
	#endif

	hid_hw_close(hdev);
	hid_hw_stop(hdev);
	hid_info(hdev, "end remove\n");
